    ...
    return (
        <div className="fixed inset-0 z-[100] bg-black/40 backdrop-blur-sm flex items-center justify-center p-3 md:p-4" onClick={onClose}>
            ...
            >
                ...
                {/* --- CONTEÚDO --- */}
                {activeTab === 'post' ? (
                    postStep === 1 ? ( //botão de postar casi seja post
                    ...
                    ) : postStep === 2 ? ( //botão de avançar caso selecione a comunidade
                        <SelectDestination 
                            onBack={() => setPostStep(1)}
                            onPublishPost={() => {
                                onClose();
                            }}
                            onAdvanceCommunity={() => setPostStep(3)} // Avança para a tela de comunidades
                        />
                    ) : (
                        <SelectCommunity 
